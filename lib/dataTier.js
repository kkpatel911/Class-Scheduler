...
async function clearClassDataBySubjectAndTerm(dirtySubject, newTerm) {
  await client.query("DELETE FROM meetingTime WHERE classID IN (SELECT classID FROM class WHERE term < $1 AND Subject = $2);", [newTerm, dirtySubject], (err, res) => {
    if (err) throw err;
  });
  await client.query("DELETE FROM class WHERE term < $1 AND Subject = $2;", [newTerm, dirtySubject], (err, res) => {
    if (err) throw err;
  });
}
